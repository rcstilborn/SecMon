...
Stream::~Stream() {
	//	std::cout << "Stream::~Stream() - destructing" << std::endl;
...
void Stream::registerNewConnection(http::connection_ptr conn){
	...
	//		//	std::ifstream is("/home/richard/Dev/OpenCV/opencv-2.4.8/samples/c/lena.jpg", std::ios::in | std::ios::binary);
	...
	//		// Create a test image, convert to jpg and send to browser
	//		cv::Mat image2(cv::Size(640, 480), CV_8UC3, cv::Scalar(std::rand() % 255, std::rand() % 255, std::rand() % 255));
	...
	//
	//		cv::Mat image3(cv::Size(640, 480), CV_8UC3, cv::Scalar(std::rand() % 255, std::rand() % 255, std::rand() % 255));
