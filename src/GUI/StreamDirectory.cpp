...
//void StreamDirectory::addStream(const std::string& name, boost::signals2::signal<void (boost::shared_ptr<std::vector<unsigned char>>)> signal){
//    boost::lock_guard<boost::mutex> guard(stream_list_mtx_);
...
bool StreamDirectory::handleValidStream(const std::string& streamRequest, http::connection_ptr conn) {
    ...
    try {
        ...
        LOG(WARNING) << "StreamDirectory::handleValidStream() - Could not convert filename to integer " << filename << ". " << e.what();
        return false;
    }
